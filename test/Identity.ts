...
describe("Identity - create", () => {
  it("should fail with Insufficient funds.", async () => {
    await expect(contract.create("test", "pfp")).to.be.revertedWith(
      "Insufficient funds to create a Profile."
    );
  });

  it("should create an Identity", () => {
    const tx = contract.create("test", "pfp", {
  ...
  it("should fail it the sender already has a profile", async () => {
    await expect(
      contract.create("test", "pfp", { value: ethers.parseEther("5") })
    ).to.be.revertedWith("Profile already exists for this wallet.");
  });
  ...
  it("should fail if the handle is already taken", async () => {
    ...
    await expect(
      contract
        .connect(acc)
        .create("test", "pfp", { value: ethers.parseEther("5") })
    ).to.be.revertedWith("Handle is already taken.");
  });
});
